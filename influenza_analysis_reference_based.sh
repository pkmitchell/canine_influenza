...
echo github: @drothen15
echo =========================================================================
echo
sleep 5s


echo -n 'enter indexed reference genome PATH: '
read reference
...
echo
echo ------------------------------------------------------------------------------------
echo Step 5 Reference-based Assembly with Bowtie2 Package
echo ------------------------------------------------------------------------------------
echo

rm -r -f reference-based-assembly
rm -f ref-assembly.cmds
rm -f sam-sort.cmds
rm -f sam-index.cmds

mkdir reference-based-assembly
cd trimmomatic-output/

for R1 in *R1_001.cor.paired*
do
   R2=${R1//R1_001.cor.paired.fq/R2_001.cor.paired.fq}
   output=${R1//_L001_R1_001.cor.paired.fq/_output.sam}
   outbam=${R1//_L001_R1_001.cor.paired.fq/_output.bam}
   outcall=${R1//_L001_R1_001.cor.paired.fq/_calls.vcf.gz}
   outcon=${R1//_L001_R1_001.cor.paired.fq/_consensus.fa}
   echo "bowtie2 -x $reference -1 ../trimmomatic-output/$R1 -2 ../trimmomatic-output/$R2 -S $output" >> ref-assembly.cmds
   echo "samtools sort $output > $outbam" >> sam-sort.cmds
   echo "samtools index $outbam" >> sam-index.cmds
   echo "bcftools mpileup -Ou -f $reffasta $outbam | bcftools call -mv -Oz -o $outcall" >> bcf.cmds
   echo "tabix $outcall" >> tab.cmds
   echo "cat $reffasta | bcftools consensus $outcall > $outcon" >> consensus.cmds
done

cat ref-assembly.cmds
cat sam-sort.cmds
cat sam-index.cmds
cat bcf.cmds
cat tab.cmds
...
chmod u+x ref-assembly.cmds
chmod u+x sam-sort.cmds
chmod u+x sam-index.cmds
chmod u+x bcf.cmds
chmod u+x tab.cmds
chmod u+x consensus.cmds

mv ref-assembly.cmds ../reference-based-assembly
mv sam-sort.cmds ../reference-based-assembly
mv sam-index.cmds ../reference-based-assembly
mv bcf.cmds ../reference-based-assembly
mv tab.cmds ../reference-based-assembly
mv consensus.cmds ../reference-based-assembly

cd ../reference-based-assembly
...
rm -r -f consnsus-fasta
mkdir consensus-fasta
mv *.fa consensus-fasta/
...
mkdir final_consensus_fastas
mv *FINAL* final_consensus_fastas
