...
## Step 5: Classifying Reads using Kraken2 (remove all non-viral seqeuences)
echo
echo --------------------------------------------------------------------------------
echo Step 5 Classifying Reads with Kraken, removing non-viral reads and contamination
echo --------------------------------------------------------------------------------
echo

rm -r -f kraken-output
rm -f kraken.cmds
mkdir kraken-output
cd lighter-output/

for R1 in *R1_001.cor*
do
   R2=${R1//R1_001.cor.fq/R2_001.cor.fq}
   classified=${R1//.cor.fq.gz/.cor.classified#.fq}
   output=${R1//_L001_R1_001.cor.fq.gz/_kraken-viral-only-classified}
   report=${R1//_L001_R1_001.cor.fq.gz/_kraken-viral-only-report}
   #echo $output $report
   echo "kraken2 --db /workdir/dr575/kraken-viral-only/ --paired --classified-out $classified --fastq-input lighter-output/$R1 lighter-output/$R2 --use-names --output $output --report $report" >> kraken.cmds
done

cat kraken.cmds
chmod u+x kraken.cmds
mv kraken.cmds ../
cd ..

./kraken.cmds

mv *_kraken-* kraken-output/
mv *classified* kraken-output/
cd kraken-output/


for i in *L001_R1_001.cor.classified_2.fq
do
   mv "$i" ${i/L001_R1_001.cor.classified_2.fq/L001_R2_001.cor.classified.fq}
done

for a in *L001_R1_001.cor.classified_1.fq
do
   mv "$a" ${a/L001_R1_001.cor.classified_1.fq/L001_R1_001.cor.classified.fq}
done
...
for R1 in *R1_001.cor.classified.fq
do
   R2=${R1//R1_001.cor.classified.fq/R2_001.cor.classified.fq}
   wd=${R1//L001_R1_001.cor.classified.fq/trinity-output}
   #echo $R1 $R2
   echo "Trinity --seqType fq --max_memory 200G --CPU 40 --left kraken-output/$R1 --right kraken-output/$R2 --output $wd/ --normalize_max_read_cov 50" >> trinity.cmds
done
